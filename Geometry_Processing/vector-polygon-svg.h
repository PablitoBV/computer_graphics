...
public:
    ...
    double area() {
    int edges = this->vertices.size();
    if (edges < 3) return 0.0; //if less than three edges, no area
    double val = 0.0;
    for (uint i = 0; i < edges; i++) {
        val += vertices[i][0]*vertices[i + 1][1] - vertices[i][1]*vertices[i + 1][0];
    }
    ...
    int n = vertices.size();
    for (int i = 0; i < n - 1; i++) {
        xpart += (vertices[i][0] + vertices[i+1][0]) *(vertices[i][0]*vertices[i+1][1] -vertices[i+1][0]*vertices[i][1]);
        ypart += (vertices[i][1] + vertices[i+1][1]) *(vertices[i][0]*vertices[i+1][1] -vertices[i+1][0]*vertices[i][1]);
    }
    xpart += (vertices[n-1][0] + vertices[0][0]) *(vertices[n-1][0]*vertices[0][1] -vertices[0][0]*vertices[n-1][1]);
    ypart += (vertices[n-1][1] + vertices[0][1]) *(vertices[n-1][0]*vertices[0][1] -vertices[0][0]*vertices[n-1][1]);
    Vector ctr(-xpart, -ypart, 0.0);
    return ctr/(6.0*area());
    }
...
void save_svg(const std::vector<Polygon> &polygons, std::string filename, std::string fillcol = "random_string") {
